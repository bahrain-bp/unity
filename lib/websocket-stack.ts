...
export class WebSocketStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: WebSocketStackProps) {
    ...
    const strokesTable = props.dbStack.whiteboardStrokesTable;
    ...
    const clearBoardFn = new NodejsFunction(this, "ClearBoardFn", {
  runtime: lambda.Runtime.NODEJS_18_X,
  entry: path.join(__dirname, "../lambda/whiteBoard/clearBoard.ts"),
  handler: "handler",
  environment: {
    ACTIVE_CONNECTIONS_TABLE: activeConnectionsTable.tableName,
    STROKES_TABLE: strokesTable.tableName,
    WS_ENDPOINT: wsEndpoint,
  },
  timeout: cdk.Duration.seconds(10),
});
...
const broadcastStrokeFn = new NodejsFunction(
  this,
  "BroadcastStrokeFn",
  {
    runtime: lambda.Runtime.NODEJS_18_X,
    entry: path.join(
      __dirname,
      "../lambda/whiteBoard/broadcastStroke.ts"
    ),
    handler: "handler",
    environment: {
      ACTIVE_CONNECTIONS_TABLE: activeConnectionsTable.tableName,
    ...
    const sendHistoryFn = new NodejsFunction(this, "SendHistoryFn", {
      runtime: lambda.Runtime.NODEJS_18_X,
      entry: path.join(__dirname, "../lambda/whiteBoard/sendHistory.ts"),
      handler: "handler",
      environment: {
        STROKES_TABLE: strokesTable.tableName,
      },
    ...
    strokesTable.grantReadData(sendHistoryFn);
...
const connectionsArn = `arn:aws:execute-api:${region}:${account}:${webSocketApi.apiId}/${webSocketStage.stageName}/POST/@connections/*`;
    ...
    activeConnectionsTable.grantReadData(broadcastStrokeFn);
    strokesTable.grantWriteData(broadcastStrokeFn);
    strokesTable.grantReadData(sendHistoryFn);

broadcastStrokeFn.addToRolePolicy(
  new iam.PolicyStatement({
    actions: ["execute-api:ManageConnections"],
    resources: [connectionsArn],
  })
    ...
    activeConnectionsTable.grantReadData(clearBoardFn);
strokesTable.grantReadWriteData(clearBoardFn);

clearBoardFn.addToRolePolicy(
  new iam.PolicyStatement({
    actions: ["execute-api:ManageConnections"],
    resources: [connectionsArn],
  })
...
webSocketApi.addRoute("clearBoard", {
  integration: new integrations.WebSocketLambdaIntegration(
    "ClearBoardIntegration",
    clearBoardFn
  ),
});
    ...
    webSocketApi.addRoute("sendStroke", {
  integration: new integrations.WebSocketLambdaIntegration(
    "SendStrokeIntegration",
    broadcastStrokeFn
  ),
});
    webSocketApi.addRoute("requestHistory", {
      integration: new integrations.WebSocketLambdaIntegration(
        "RequestHistoryIntegration",
        sendHistoryFn
      ),
    });
