...
{
    [Header("References")]
    public Transform playerRoot;        // the object that rotates yaw (same object as FPController)
    public Transform cameraTransform;   // Main Camera (child of playerRoot)
    public Transform target;            // PeccyHeadAnchor (recommended)
    ...
    void LateUpdate()
    {
        ...
        // After yaw adjustment, recompute direction in camera space
        Vector3 dirCamSpace = cameraTransform.InverseTransformDirection(dirWorld.normalized);

        // Pitch is rotation around X: negative because looking up is -X in typical FPS setups
        ...
        // Optional: stop focusing when close enough to prevent micro jitter
        ...
        {
            // Once aligned, stop updating to avoid tiny corrections that look like shake
            focusing = false;
        }
    }
    ...
    public void SnapFocus()
    {
        ...
        // After snapping, we usually don't need to keep focusing
        focusing = false;
    }
}
